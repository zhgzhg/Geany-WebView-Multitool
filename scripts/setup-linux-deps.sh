...
# geany, webkit2gtk-4.1 (the GTK3 API of WebKitGTK) and json-glib.
set -e

if command -v dnf >/dev/null 2>&1; then
	PM="dnf"
	CMD="dnf install -y gcc gcc-c++ meson ninja-build geany geany-devel webkit2gtk4.1-devel json-glib-devel"
elif command -v apt-get >/dev/null 2>&1; then
	PM="apt"
	CMD="apt-get install -y build-essential meson ninja-build geany libgeany-dev libwebkit2gtk-4.1-dev libjson-glib-dev"
elif command -v pacman >/dev/null 2>&1; then
	PM="pacman"
	# Arch ships headers in the main packages; no -devel splits.
	CMD="pacman -S --needed --noconfirm base-devel meson ninja geany webkit2gtk-4.1 json-glib"
else
	echo "Unsupported distro: install manually — a C/C++ toolchain, meson, ninja," >&2
	echo "and devel packages for: geany, webkit2gtk-4.1, json-glib." >&2
	exit 1
fi
